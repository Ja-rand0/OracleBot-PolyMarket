...
print(
    f"  {acc}*{FITNESS_W_ACCURACY} + {edge}*{FITNESS_W_EDGE}"
    f" - {fpr}*{FITNESS_W_FALSE_POS} - ({complexity}/{TOTAL_METHODS})*{FITNESS_W_COMPLEXITY}"
)
print(
    f"  = {acc * FITNESS_W_ACCURACY:.5f} + {edge * FITNESS_W_EDGE:.5f}"
    f" - 0 - {(complexity / TOTAL_METHODS) * FITNESS_W_COMPLEXITY:.5f}"
)
print(f"  = {fitness_manual:.4f}  (reported: 0.3546)")

# T17+P24 case: edge=0.0317, complexity=2
acc2, edge2, fpr2, comp2 = 1.0, 0.0317, 0.0, 2
f2 = acc2 * FITNESS_W_ACCURACY + edge2 * FITNESS_W_EDGE - fpr2 * \
    FITNESS_W_FALSE_POS - (comp2 / TOTAL_METHODS) * FITNESS_W_COMPLEXITY
...
for e in [0.0, 0.01, 0.03, 0.05, 0.10, 0.20]:
    f = 1.0 * FITNESS_W_ACCURACY + e * FITNESS_W_EDGE - 0.0 * \
        FITNESS_W_FALSE_POS - (2 / TOTAL_METHODS) * FITNESS_W_COMPLEXITY
